...
- Picker zadań w focus window obciął listę przy większej liczbie pozycji i utrudniał scroll — strefa scrollowania powiększona z 200 → 400 px, okno picker'a 320 → 480 px.

### Added

- Opcja `Zakończ bez wybierania nowego zadania` jako pierwsza pozycja na liście pickera (wcześniej była tylko jako stopka).
...
- Energy tracker — pierwszy check-in dnia jest losowany w przedziale 5-15 min od pierwszej aktywności (idle <60 s), nie z normalnego interwału 60-120 min. Dzień rozliczeniowy zaczyna się o 6:00 lokalnie (a nie o północy), więc praca po północy nie wyzwala nowego "first activity". Wykrywanie raz na dzień, persisted w `EnergyTrackerConfig.lastFirstActivityDate`.
...
- `top5 qt add` now uses shared `qtCode()` formatter, consistent with all other quick-task commands
...
- `isSafeFilename` now rejects empty strings and filenames containing null bytes (`\0`), closing two edge-case bypass vectors
